...
from utils import MaskGenerator, Masker, denormalize
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for i, (images, target, mask) in enumerate(loader):        
        ...
        if i == im_interest:
            ...
            psnr_den_list[0] = psnr_images.cpu().detach().numpy().astype('float32')            
        ...
        if i == im_interest:
            ...
            # plot clean, noisy images, and output images after each iteration
            plt.figure(figsize=(20, 20))       
            for k in range(args.niters + 3): 
                output_cpu = tensor2np(output_denoised_im_list[k])
                plt.subplot(1, args.niters + 3, k + 1)
                if k == 0:
                    title = "Clean"
                elif k == 1:              
                    title = f"Noisy: {psnr_den_list[0]:.2f}"  
                else:
                    title = fr"$\mathbf{{O}}_{{{k-2}}}$: {psnr_den_list[k-1]:.2f}"              
                plt.title(title, fontsize=8)
                plt.imshow(output_cpu)
                plt.axis('off')
            plt.show();  
            plt.figure(figsize=(20, 20))
            for k in range(args.niters + 2):          
                output_cpu = tensor2np(output_reconstructed_im_list[k])             
                plt.subplot(1, args.niters + 2, k + 1)
                if k == 0:
                    title = f"Masked: {psnr_rec_list[0]:.2f}"
                else:
                    title = fr"$\mathbf{{O}}_{{{k-1}}}^{{\prime}}$: {psnr_rec_list[k]:.2f}"
                plt.title(title, fontsize=8)
                plt.imshow(output_cpu)
                plt.axis('off')
            plt.show();      
